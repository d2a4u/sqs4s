...
object Credentials {
  ...
  def envVarResource[F[_]: Sync]: Resource[F, Credentials[F]] =
    Resource.liftF(envVar[F])

  private def envVar[F[_]: Sync]: F[Credentials[F]] =
    Sync[F].delay {
  ...
  def sysPropResource[F[_]: Sync]: Resource[F, Credentials[F]] =
    Resource.liftF(Sync[F].suspend(sysProp[F]))

  private def sysProp[F[_]: Sync]: F[Credentials[F]] =
    Sync[F].delay {
  ...
  def instanceMetadata[F[_]: Concurrent: Clock: Timer](
    ...
    val refF = refresh.flatMap(Ref.of[F, CredentialResponse])
